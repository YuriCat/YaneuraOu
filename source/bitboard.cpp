...
Bitboard PAWN_DROP_MASK_BB[0x20]; // pには1～5筋のデータが入っている。
...
void Bitboards::init()
{
	...
	for (Rank rank = RANK_1; rank <= RANK_5 ; ++rank)
	{
		...
		{
			// iはsqに駒をおいたときに、その筋の2段～4段目の升がemptyかどうかを表現する値なので
	...
	for (int i = 0; i < 0x20; ++i)
	{
		...
		PAWN_DROP_MASK_BB[i].p = b.p; // 1～5筋
	}
